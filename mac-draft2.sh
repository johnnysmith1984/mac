...
sudo macchanger -s $answer | cut -b -78 && sleep .4s && echo ""
...
*) sleep .6s && echo "" && echo "Invalid Option";; esac
echo "" && sleep .6s && sudo ip link set dev $answer up && sleep .6s
echo "" && echo -n "Done? y/n: " && read i && case $i in
y) echo "" && sleep .6s && break;; *) ;; esac; done
