...
sudo macchanger -s $answer && sleep .6s && echo ""
...
0) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -m 00:24:f3:69:b4:aa $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
1) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -p $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
2) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -a $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
3) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -r $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
4) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -e $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
5) sudo ip link set dev $answer down && sleep .6s
   echo "" && sudo macchanger -r -b $answer | sed 's/Current MAC: /Previous MAC:/g' | sed "2 d";;
6) echo "" && echo "            XX:XX:XX:XX:XX:XX"
   echo -n "Specify MAC:" && read answer2
   sudo ip link set dev $answer down && echo "" && sleep .6s
